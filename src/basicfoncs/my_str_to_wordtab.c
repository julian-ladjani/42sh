...
** Last update Sun Apr  9 22:35:31 2017 Ladjani Julian
*/
...
int	tab_count(char *str, char sep)
{
  int	tabc;
  int	c;
...
int	char_count(char *str, char sep)
{
  int	c;
  ...
  while (*str != '\0')
      ...
      if (*str != '\0')
	{
	  if ((my_array[tab] = malloc(char_count(str, sep) + 2)) == NULL)
	    exit(84);
	  my_strncpy(my_array[tab], str, char_count(str, sep));
	  tab++;
